  ...
  return (
    ...
    `}>
        ...
        <div className="flex items-center gap-2.5 min-w-0 pr-2">
          ...
          {isLoadingProfile ? (
            ...
            <div className="flex h-8 w-8 items-center justify-center rounded-lg overflow-hidden shrink-0 relative">
              <Image src="/Logo.webp" width={32} height={32} className="h-full w-full object-contain" alt={displayName} />
            </div>
          )}
      ...
      <div className={`flex flex-col flex-1 overflow-hidden ${!isMobileExpanded ? 'hidden md:flex' : 'flex'}`}>
            ...
            <div className="flex items-center gap-2.5 w-full">
              {isLoadingProfile ? (
                ...
                <div className={`flex ${isDashboard ? 'h-8 w-8' : 'h-9 w-9'} items-center justify-center rounded-lg overflow-hidden shrink-0 relative`}>
                  <Image src="/Logo.webp" width={36} height={36} className="h-full w-full object-contain" alt={displayName} />
                </div>
              )}
        ...
        {/* ── Boundaries Section (Collapsible) ── */}
        {boundaries.length > 0 && (
          <div className={isDashboard ? "px-4 pt-1 pb-2" : "px-6 pt-2 pb-3"}>
            ...
            >
              <h2 className="text-[12px] font-bold text-slate-600 dark:text-slate-400 flex items-center gap-1.5 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                <Layers className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
                Batas Wilayah ({boundaries.length})
              </h2>
              <button className="text-slate-400 group-hover:text-cyan-600 dark:group-hover:text-white transition-colors p-0.5">
                {isBoundariesExpanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
              </button>
            </div>
                ...
                {boundaries.map((b) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        onClick={(e) => { e.stopPropagation(); toggleBoundaryVisibility(b.id, !b.is_visible); }}
